...
inline static void qsc_memutils_clear128(void* output)
{
...
inline static void qsc_memutils_clear256(void* output)
{
#if defined(QSC_SYSTEM_HAS_AVX2)
	_mm256_storeu_si256((__m256i*)output, _mm256_setzero_si256());
#else
	qsc_memutils_clear128((char*)output);
	qsc_memutils_clear128((char*)output + 16);
#endif
}

inline static void qsc_memutils_clear512(void* output)
{
#if defined(QSC_SYSTEM_HAS_AVX512)
	_mm512_storeu_si512((__m512i*)output, _mm512_setzero_si512());
#else
	qsc_memutils_clear256((char*)output);
	qsc_memutils_clear256((char*)output + 32);
#endif
}
...
#	endif
		...
		if (length >= SMDBLK)
		{
			...
			{
#	if defined(QSC_SYSTEM_HAS_AVX512)
				qsc_memutils_clear512(output + pctr);
#	elif defined(QSC_SYSTEM_HAS_AVX2)
				qsc_memutils_clear256(output + pctr);
#	elif defined(QSC_SYSTEM_HAS_AVX)
				qsc_memutils_clear128(output + pctr);
#	endif
				pctr += SMDBLK;
			}
		}
		...
		if (pctr != length)
		{
			memset(output + pctr, 0x00, length - pctr);
		}
	}
}
...
inline static void qsc_memutils_copy256(const uint8_t* input, uint8_t* output)
{
	...
	qsc_memutils_copy128(input + 16, output + 16);
#endif
}
...
inline static void qsc_memutils_copy512(const uint8_t* input, uint8_t* output)
{
	...
	qsc_memutils_copy256(input + 32, output + 32);
#endif
}
...
#	endif
		...
		if (pctr != length)
		{
			memcpy(output + pctr, input + pctr, length - pctr);
		}
	}
}
...
inline static void qsc_memutils_setval256(uint8_t* output, uint8_t value)
{
	...
	qsc_memutils_setval128(output + 16, value);
#endif
}
...
inline static void qsc_memutils_setval512(uint8_t* output, uint8_t value)
{
	...
	qsc_memutils_setval256(output + 32, value);
#endif
}
...
#	endif
		...
		if (length >= SMDBLK)
		{
			...
			{
#if defined(QSC_SYSTEM_HAS_AVX512)
				qsc_memutils_setval512(output + pctr, value);
#elif defined(QSC_SYSTEM_HAS_AVX2)
				qsc_memutils_setval256(output + pctr, value);
#elif defined(QSC_SYSTEM_HAS_AVX)
				qsc_memutils_setval128(output + pctr, value);
#endif
				pctr += SMDBLK;
			}
		}
		...
		if (pctr != length)
		{
			memset(output + pctr, value, length - pctr);
		}
	}
}
